...
getPath = (wire, path) => {
  ...
  path.set(`${curX},${curY}`, dist)

  wire.forEach((step) => {
    const dir = step.substring(0, 1)
    const length = Number(step.substring(1, step.length))
...
path1.forEach((val, key) => {
  if ( path2.has(key) ) { 
    crossed.push([key, val + path2.get(key)])
  }
})

let lowestDist = null

crossed.shift() // remove the 0,0

crossed.forEach((cross) => {
  const dist = cross[1]
